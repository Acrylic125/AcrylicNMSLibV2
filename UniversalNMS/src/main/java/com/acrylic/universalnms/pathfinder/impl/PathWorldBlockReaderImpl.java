...
public class PathWorldBlockReaderImpl implements PathWorldBlockReader {
    ...
    private final Map<BlockKey, BlockAnalyzer> analyzerMap;

    public PathWorldBlockReaderImpl(@NotNull World world) {
        this(world, new HashMap<>());
    }

    public PathWorldBlockReaderImpl(@NotNull World world, @NotNull Map<BlockKey, BlockAnalyzer> analyzerMap) {
        this.world = world;
    ...
    @Override
    public BlockAnalyzer getBlockAnalyzerAt(int x, int y, int z) {
        Block block = getBlockAt(x, y, z);
        BlockKey blockKey = new BlockKey(block);
        BlockAnalyzer blockAnalyzer = analyzerMap.get(blockKey);
        if (blockAnalyzer == null) {
            blockAnalyzer = NMSLib.getNMSUtilityFactory().getNewBlockAnalyzer(block);
